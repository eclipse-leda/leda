...
## Integrating RAUC with your bootloader
...
The only Leda quickstart image that uses GRUB as a a bootloader is for qemux86_64. It directly uses [meta-rauc-community/meta-rauc-qemux86/](https://github.com/rauc/meta-rauc-community/tree/master/meta-rauc-qemux86) meta-layer.

### U-Boot

The rpi4-64 target, similarly to qemux86_64, directly uses [meta-rauc-community/meta-rauc-raspberrypi](https://github.com/rauc/meta-rauc-community/tree/master/meta-rauc-raspberrypi) which also has a great README, explaining the details of the integration. This meta-layer includes quite a lot of rpi4-64 specific recipes and depends on the fact that the meta-raspberrypi layer provides a recipe for custom boot scripts. Such recipe, in general, for U-Boot based targets is not available and has to be manually created. That is why, [meta-rauc-community/meta-rauc-sunxi](https://github.com/rauc/meta-rauc-community/tree/master/meta-rauc-sunxi) might provide a better example in a more general case.
...
If a custom boot.scr that was compiled with mkimage is in the same partition as the U-Boot binary, U-Boot will run that on boot. This is the point of integration of RAUC and U-Boot. All concepts behind setting up such a script are explained in the [rauc doccumentation](https://github.com/rauc/rauc/blob/8216e0df6c716a87406df676295af94b77814e5e/docs/integration.rst#id27).
...
As it can be seen in the examples, after the "slot retries counting" part of the script is done, the `root=` and `rauc.slot=` kernel commandline arguments are set, the U-Boot environment is saved in the VFAT boot partition as uboot.env and the selected image is booted.

Then, from userspace RAUC reads the kernel commandline (/proc/cmdline) and the uboot.env (through the fw_printenv/fw_setenv utilities) and decides (based on that) whether a slot is "Good" or "Bad".
...
This however, is not enough since these utilities, in turn have to be configured to know where U-Boot stores its environment. This done through the /etc/fw_env.config file. An example of such file with all of its different variations can be seen here [fw_env.config](https://github.com/ARM-software/u-boot/blob/master/tools/env/fw_env.config).

The one for Leda quickstart contains the single line:
...
The RAUCK mark-good-service should should successfully go through and mark both RAUC slots as "GOOD"